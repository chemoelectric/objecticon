 ...
 * doesn't exist.  A trailing '/' is always added.
 */
char *canonicalize_path(char *path)
{
    static char result[PATH_MAX + 1]; /* +1 so we can add a trailing / */
#if UNIX
    static char currentdir[PATH_MAX];
    ...
    /* The test for *path means that the empty string resolves to the cwd */
    if (*path && chdir(path) == -1)
        return 0;

    if (!getcwd(result, sizeof(result))) {
        fprintf(stderr, "getcwd return 0 - dir too long:%s", path);
        exit(1);
    }
