...
void
frinit(Frame *f, Rectangle r, Font *ft[NFONT], Image *b, Image *cols[NCOL])
{
        memmove(f->fonts, ft, sizeof(f->fonts));
        memmove(f->cols, cols, sizeof(f->cols));
	f->font = f->fonts[REGULAR_FONT];
	f->display = b->display;
	f->maxtab = 8*stringwidth(f->font, "0");
	f->nbox = 0;
	f->nalloc = 0;
	f->nchars = 0;
	f->nlines = 0;
